...
class cwater:
    ...
    def run(self, zscale: int) -> bool:
        ...
        seedlist = []
        seedlist.append([5253, 11352])
        seedlist.append([5721, 7058])
        ...
        maskz = self.gen_buff_resistance(mxz_dem, 500, seedlistz, 100.0, 1.1)

        mxz_dem = mxz_dem + maskz * 20
        fldz = self.region_grow(mxz_dem, 25, seedlistz)
    ...
    def gen_buff_resistance(
        ...
        Function:
            ...
            the returned resistance buffer will add to dem to simply rise the altitude as the resistance.
    ...
    def region_grow(self, img: np.ndarray, thre: float, slist: list) -> bytearray:
        ...
        while len(slist0) > 0:
            ...
            for i in range(0, ndir):
                ...
                # is out of edge
                if (cor[0] >= xmax) or (cor[0] < 0) or (cor[1] >= ymax) or (cor[1] < 0):
                    continue
...
if __name__ == "__main__":
    filepath = "/home/tq/zgq/zdata/flood/2014/maskout/dem"
    cw = cwater(filepath)
    cw.run(20)
